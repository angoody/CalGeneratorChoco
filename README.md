...
L'objet Constraint est obligatoire dans l'objet Problem.
Dans l'objet Constraint, aucune contrainte n'est obligatoire (l'objet Constraint peut être vide).
Une seul valeur par défaut existe pour constraintPrerequisite, elle ne peut être enlevé par contre si une valeur est renseignée elle remplacera la valeur par défaut pour cet appel. 

Si une contrainte est renseigné alors leurs valeurs obligatoire et les valeurs par défaut dépendent de leur type d'objet.
