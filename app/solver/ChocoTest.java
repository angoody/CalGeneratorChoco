...
public class ChocoTest
{
    ...
    public static List<Calendar> solve()
    {
        ...
        List<Calendar> calendars = solver.solve();
    ...
    public static Generator getGeneratorInput()
    {
        Generator problem = new Generator(initProblem());
        problem.setNumberOfCalendarToFound(5);
        return problem;
    ...
    public static CalendarVerify verify()
    {
        Verify problem = getVerifyInput();
        ChocoVerify solver = new ChocoVerify(problem);
        CalendarVerify calendar = solver.solve();
    ...
    public static Verify getVerifyInput()
    {
        Generator problem = new Generator(initProblem());
        problem.setNumberOfCalendarToFound(1);
        ...
        Verify returnedObject = new Verify();
        returnedObject.setConstraints(problem.getConstraints());
        returnedObject.setPeriodOfTraining(problem.getPeriodOfTraining());

        List<Module> modules = new ArrayList<>();

        problem.getModuleOfTraining().stream().forEach( m ->
                m.setListClasses(m.getListClasses().stream().filter(c -> calendars.get(0).getCours().stream().filter(c2 -> c2.getIdClasses().contentEquals(c.getIdClasses())).count() > 0).collect(Collectors.toList())));

        returnedObject.setModuleOfTraining(problem.getModuleOfTraining());
...
*/
        ...
        ConstraintPriority<TrainingFrequency> frequence = new ConstraintPriority<>(3, new TrainingFrequency());
        ...
        problem.getConstraints().setAnnualNumberOfHour(new ConstraintPriority<>(8, 1200));
        problem.getConstraints().setMaxDurationOfTraining(new ConstraintPriority<>(8, 104));
